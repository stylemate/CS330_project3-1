...
process_execute (const char *file_name)
{
  ...
  /* Parse *file_name to get the first tocken; file name. */
  char *save_ptr;
...
/* Waits for thread TID to die and returns its exit status.  If
   ...
   exception), returns -1(case a).  If TID is invalid or if it
   was not a child of the calling process, or if process_wait()
   has already been successfully called for the given TID,
   returns -1 immediately, without waiting(case b). */
int
process_wait (tid_t child_tid UNUSED)
{
  struct child_process *child = search_child_process(child_tid);
  /* For cases a and b. */
  if(child == NULL) return -1;
  if(child->called) return -1;
  /* Flag this child process has been called */
  child->called = true;

  /* Waiting till its child process killed and reap it*/
  sema_down(&child->killed_sema);
...
process_exit (void)
{
  ...
  /* Close all the files current process has */
  lock_acquire(&file_lock);
  close_files();
  if(cur->executing != NULL) file_close(cur->executing);
  lock_release(&file_lock);

  /* Destroy the current process' child processes.
     Built in syscall.c. */
  child_process_destroy();

  /* Kernel kills this process, make thread waiting
     this work awake. */
  if(is_running(cur->parent) && cur->child && cur->executing){
    cur->child->killed = true;
    sema_up(&cur->child->killed_sema);
  }
...
setup_stack (void **esp, const char *file_name, char *save_ptr, uint8_t *upage)
{
  ...
  /* Push the address of string plus a null pointer sentinel,
     on the stack, in right-to-left order. */
  int argc = 0, argc_max = 4, i;
  char *token, **argv;
  void *tmp;
  ...
  for(token = (char *) file_name ; token != NULL ;
    ...
    *esp -= (strlen(token) + 1);
    ...
    argc += 1;
    if(argc >= argc_max){
      /* If given memory to argv isn't enough, re-allocate the memory. */
      argc_max += 2;
      char **new_argv = realloc (argv, (sizeof(char *) * argc_max));
      /* Code seg for memory lacking case. */
      if (new_argv == NULL){
        exit(-1);
      }
      else{
  ...
  /* Write meaningless data for word-align */
  argv[argc] = 0;
  i =(size_t)*esp % 4;
  if(i != 0){
    *esp -= i;
  ...
  /* Push from argv[argc] through argv[0], argv, argc,
  ...
  tmp = *esp;
  *esp -= sizeof(char **);
  memcpy(*esp, &tmp, sizeof(char **)); // argv
  *esp -= sizeof(int);
  memcpy(*esp, &argc, sizeof(int)); // argc
  *esp -= sizeof(void *);
  memcpy(*esp, &argv[argc], sizeof(void *)); // fake return address
  free(argv);
  return success;
}
